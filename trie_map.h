...
class TrieMap {
   ...
   public:
    ...
    V& operator[](std::string&& key) {
        auto [node, value] =
            InsertNodes(root_, std::forward<std::string>(key), 0);
        root_ = node;
        return value;
    }
    ...
    std::list<std::string> FindKeysWithPattern(
   ...
   private:
    ...
    std::pair<Node*, V&> InsertNodes(Node* node, std::string&& key, size_t i) {
        ...
        // 在 node 的 children 数组中“插入字符 c”
        // 递归地从 node->children[c] 开始插入 key[i+1..]
        auto [child, value] = InsertNodes(
            node->children[c], std::forward<std::string>(key), i + 1);
        node->children[c] = child;
        return {node, value};
    }
    ...
    Node* FindNode(Node* node, std::string&& key, size_t i) const {
        ...
        char c = key[i];
        return FindNode(node->children[c], std::forward<std::string>(key),
                        i + 1);
    }
    ...
    void Traverse(Node* node, std::string& path, std::string&& pattern,
                  size_t i, std::list<std::string>& keys) const {
        if (node == nullptr) {
            // 树枝不存在，即匹配失败
            return;
        ...
        if (c == '.') {
            ...
            for (size_t j = 0; j < kASCIICodeCount_; ++j) {
                if (node->children[j] != nullptr) {
                    ...
                    path.push_back(j);
                    // 递归地遍历 children[j]
                    Traverse(node->children[j], path,
                             std::forward<std::string>(pattern), i + 1, keys);
                    // 撤销选择
                    path.pop_back();
                }
            }
    ...
    bool Match(Node* node, const std::string& pattern, size_t i) const {
        ...
        if (c != '.') {
            ...
            // 遇到通配符
            // pattern[i]
            // 可以变化成任意字符，尝试所有可能，只要遇到一个匹配成功就返回
            for (size_t j = 0; j < kASCIICodeCount_; ++j) {
